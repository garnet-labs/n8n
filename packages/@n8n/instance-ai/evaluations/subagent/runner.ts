		...
		try {
			...
			if (verbose) {
				...
				for (const tr of toolResults) {
					...
					const detail =
						res?.success === false
							? `: ${String(Array.isArray(res.errors) ? (res.errors as string[]).join('; ') : (res.error ?? '')).slice(0, 200)}`
							: '';
					console.log(
				...
				console.log(
					`  [${testCase.id}] maxSteps=${String(maxSteps)}, steps used=${String(toolCalls.length)}, stopReason=${String((result as unknown as Record<string, unknown>).finishReason ?? (result as unknown as Record<string, unknown>).stopReason ?? '?')}`,
				);
			}
		} finally {
